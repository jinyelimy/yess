...
const ContactCard: React.FC<{
  ...
  phone: string;
  email: string;
}> = ({ name, role, tag, tagTone, phone, email }) => (
  <div
    style={{
      padding: "10px 12px",
      border: GRID_BORDER,
  ...
  >
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <div>
        ...
        <span style={{ color: "var(--text-tertiary)", fontSize: 12.5 }}> · {role}</span>
      </div>
      {tag &&
        (tagTone === "primary" ? (
  ...
  <>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 14 }}>
        <FieldInput label="예정일" mono defaultValue={c.scheduled || ""} placeholder="YYYY-MM-DD" />
        <FieldInput label="반영일" mono defaultValue={c.applied || ""}   placeholder="YYYY-MM-DD" />
      </div>

      <div style={{ marginTop: 14 }}>
        <FieldTextarea label="이슈 / 특이사항" defaultValue={c.issue || ""} />
      </div>

      <hr style={{ margin: "20px 0", border: 0, borderTop: GRID_BORDER }} />

      <div className="t-overline" style={{ marginBottom: 12 }}>환경정보</div>
      <div
        style={{
          background: "var(--bg-surface-2)",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div className="t-overline">연락처 · 2명</div>
        <Btn variant="ghost" size="sm" icon="plus">추가</Btn>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <ContactCard name="김영수" role="전산실장" tag="PRIMARY" tagTone="primary" phone="010-1234-5678" email="kim@sample.co.kr" />
        <ContactCard name="박민주" role="인사팀"   tag="YEAR_END" phone="010-9876-5432" email="park@sample.co.kr" />
      </div>
    ...
    <div style={{ padding: 20, borderTop: GRID_BORDER, display: "flex", gap: 8 }}>
      <Btn variant="primary" size="md" style={{ flex: 1, justifyContent: "center" }}>저장</Btn>
      <Btn variant="secondary" size="md">후속조치 생성</Btn>
    </div>
  </>
);
...
export default function TargetsPage() {
  ...
  const [activeRow, setActiveRow] = React.useState<Company | null>(null);
  ...
  return (
    <div style={{ maxWidth: 1500, margin: "0 auto" }}>
      <PageHeader
        title="고객사 패치 현황"
        subtitle="차수별 · 고객사별 반영 상태를 인라인 편집. 유지보수 담당자는 본인 담당만 기본 노출."
        actions={
          <>
            <Btn variant="ghost" size="md">일괄 변경</Btn>
            <Btn variant="secondary" size="md">대상 재생성</Btn>
            <Btn variant="primary" size="md" icon="plus">패치 차수 생성</Btn>
          </>
        }
      />
      ...
      <Card style={{ overflow: "hidden" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 13, tableLayout: "auto" }}>
            ...
            <tbody>
              {COMPANIES.filter((c) => {
                ...
                const active = activeRow?.id === c.id;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <input type="checkbox" onClick={(e) => e.stopPropagation()} style={{ accentColor: "var(--accent-500)" }} />
                    </Td>
                    <Td
                      sticky
                      left={36}
        ...
        >
          ...
          <div style={{ flex: 1 }}></div>
          <Btn variant="ghost" size="sm" icon="chevR" style={{ flexDirection: "row-reverse" }}>이전</Btn>
